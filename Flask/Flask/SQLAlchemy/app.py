...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///studentsdata.sqlite3'
...
#    # raise exception
  if request.method == 'POST':
      if not request.form['id']:
          ...
          flash('Please enter ID Number', 'error')
      else:

        ID = request.form['id']
        IDD = students.query.filter_by(id=ID).first()
        if IDD:
                    db.session.delete(IDD)
                    db.session.commit()
                    flash('Record was successfully deleted', 'error')
                    return redirect(url_for('show_all'))
        else:
